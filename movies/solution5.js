...
output: object/hash map
create a function that takes the movie data
create an emptry answer object
create a loop 
    check if the rating exists
    if it exists and 1 to the count for that rating
    else add new rating and set the count to 1
after loop return answer map
*/

function ratingsCount(movies) {
  let answer = new Map();
  for (const movie of movies) {
    if (answer.has(movie.Rating)) {
      answer.set(movie.Rating, answer.get(movie.Rating) + 1);
    } else {
      answer.set(movie.Rating, 1);
    }
  }
  return answer;
}

console.log(ratingsCount(movies));
